...
final class McpHeader
{
    ...
    public static function nameFor(string $method, ?array $params): ?string
    {
        switch ($method) {
            ...
            case 'prompts/get':
                ...
                break;
            case 'resources/read':
                $value = $params['uri'] ?? null;
                break;
            case 'tasks/get':
            case 'tasks/update':
            case 'tasks/cancel':
                $value = $params['taskId'] ?? null;
                break;
            default:
                $value = null;
                break;
    ...
    {
        if (0 !== strncmp($value, self::BASE64_PREFIX, strlen(self::BASE64_PREFIX)) || 0 !== substr_compare($value, self::BASE64_SUFFIX, -strlen(self::BASE64_SUFFIX))) {
            return $value;
        }

        $encoded = (string) substr($value, \strlen(self::BASE64_PREFIX), -\strlen(self::BASE64_SUFFIX));
    ...
    public static function annotations(array $schema, string $prefix = ''): array
    {
        ...
        foreach ($properties as $property => $definition) {
            ...
            $found = array_merge($found, self::annotations($definition, $path));
        }
