...
class SamplingMessage extends Content
{
     ...
     */
    private static function hydrateContent(array $contentData)
    {
        $contentType = $contentData['type'] ?? null;
        ...
        switch ($contentType) {
            ...
            case 'audio':
                return AudioContent::fromArray($contentData);
            case 'tool_use':
                return ToolUseContent::fromArray($contentData);
            case 'tool_result':
                return ToolResultContent::fromArray($contentData);
            default:
                throw new InvalidArgumentException(\sprintf('Invalid content type "%s" for SamplingMessage.', $contentType));
        }
    }
}
